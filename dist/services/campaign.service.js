...
export async function createCampaignService(input) {
    ...
    await Promise.all(campaign.deliveries.map((delivery) => prisma.emailDelivery.update({
...
export async function getCampaignService(campaignId) {
    ...
    const stats = campaign.deliveries.reduce((acc, delivery) => {
        acc.total += 1;
...
export async function getApplicationEmailHistoryService(applicationId) {
...
export async function markDeliverySent(deliveryId, attempts) {
...
export async function markDeliveryFailed(deliveryId, errorMessage, attempts) {
    await prisma.emailDelivery.update({
        where: { id: deliveryId },
        data: {
